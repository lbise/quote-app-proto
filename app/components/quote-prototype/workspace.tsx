...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, Check, CheckCheck, ChevronDown, Copy, FileText, List, LockKeyhole, MessageSquare, Pencil, Plus, RotateCcw, Settings2, Trash2, TriangleAlert } from 'lucide-react';
...
export function VariantB(p: LayoutProps) { return <div className="qp-layout qp-layout-b"><div className="qp-review-rail">{p.outline}</div><div className="qp-review-main">{p.toolbar}<div className="qp-split">{p.document}{p.chat}</div></div></div>; }
...
export default function QuotePrototype() {
  ...
  function openRecord(record: RecordState) {
    ...
    setMessages(record.messages); setPrevious(null); setChanged([]); setSectionId(''); setList(false);
  }
  function loadSample(kind: 'joinery' | 'flat' | 'empty' | 'complete') {
    ...
    setRecordId(`sample-${Date.now()}`); setRevisions([]); setDraft(null); setReadRevision(null); setPrevious(null); setChanged([]); setSectionId(''); setList(false);
  ...
  function runAssistant(text: string, action = 'clarify', retry = false) {
    ...
    setTimeout(() => {
      ...
      if (action === 'fill') {
        ...
        setMessages(m => [...m, { role: 'assistant', fr: 'Ce prototype ne comprend pas les demandes libres. Utilisez un exemple ci-dessous pour essayer une modification, ou corrigez directement le devis. Aucun contenu n’a été changé.', en: 'This prototype does not interpret free-form requests. Use an example below to try a change, or edit the Quote directly. No content was changed.' }]); saveLater(); return;
      }
  ...
  const outline = <nav className="qp-outline" aria-label={t('Sections du devis', 'Quote sections')}>
    <div className="qp-outline-heading"><span>{t('Dans ce devis', 'In this Quote')}</span><span>{quote.lines.length} {t('lignes', 'lines')}</span></div>
  ...
  const chat = <section className="qp-conversation" aria-label={t('Conversation avec l’assistant', 'Conversation with assistant')}>
    <header className="qp-panel-heading"><div className="qp-assistant-heading"><MessageSquare /><div><h2>{t('Préparons votre devis', 'Prepare your Quote')}</h2><p>{t('Décrivez le travail. Affinons les détails.', 'Describe the work. Refine the details.')}</p></div></div><span className="qp-simulation-label">{t('Simulation', 'Simulated')}</span></header>
    ...
    <div className="qp-compose-area">
      {readOnly ? <div className="qp-chat-locked"><LockKeyhole /><p>{t('Cette révision est figée. Créez un brouillon pour poursuivre.', 'This revision is frozen. Create a draft to continue.')}</p></div> : <>
        <details className="qp-prompt-examples" open={messages.length < 6}><summary>{t('Essayer une demande', 'Try a request')}<ChevronDown /></summary><div>
          {valuePrompt && <button onClick={() => runAssistant(valuePrompt, 'fill')}>{t('Compléter les valeurs', 'Complete the values')}<span>{valuePrompt}</span></button>}
          {!quote.lines.length ? <button onClick={() => runAssistant(t('Habillage mural en chêne, 12 m² à 180 CHF/m².', 'Oak wall cladding, 12 m² at CHF 180/m².'), 'start')}>{t('Décrire un premier travail', 'Describe the first job')}<span>Chêne · 12 m² · 180 CHF/m²</span></button> : <button onClick={() => runAssistant(t('Applique une remise globale de 3 %.', 'Apply a 3% whole-Quote discount.'), 'discount')}>{t('Appliquer 3 % de remise', 'Apply a 3% discount')}<ArrowRight /></button>}
          {quote.sections.length > 0 && <button onClick={() => runAssistant(t(`Duplique « ${quote.sections.find(s => s.id === activeSection)?.title} ». Je n’ai pas encore les nouvelles quantités.`, `Duplicate “${quote.sections.find(s => s.id === activeSection)?.title}”. I do not have the new quantities yet.`), 'copy')}>{t('Dupliquer sans les quantités', 'Duplicate without quantities')}<Copy /></button>}
        </div></details>
        ...
        <p className="qp-ai-disclosure">{t('Assistant simulé. Aucune donnée transmise à une IA.', 'Simulated assistant. No data is sent to an AI.')}</p>
      </>}
    </div>
  </section>;
  ...
  const quoteDocument = <section className="qp-document-pane" aria-label={t('Devis destiné au client', 'Customer-facing Quote')}>
    ...
    <div className="qp-document-scroll" role="region" tabIndex={0} aria-label={t('Contenu du devis, défilant', 'Scrollable Quote content')}>
