...
function EditorDialog({ title, description, children, onClose }: { title: string; description: string; children: ReactNode; onClose: () => void }) {
  return <Dialog open onOpenChange={open => !open && onClose()}><DialogContent className="qp-modal" showCloseButton={false}><DialogHeader><DialogTitle>{title}</DialogTitle><DialogDescription>{description}</DialogDescription></DialogHeader>{children}</DialogContent></Dialog>;
}
function Actions({ locale, onClose }: { locale: Locale; onClose: () => void }) {
  return <DialogFooter><Button type="button" variant="outline" onClick={onClose}>{text(locale, 'Annuler', 'Cancel')}</Button><Button type="submit">{text(locale, 'Appliquer', 'Apply')}</Button></DialogFooter>;
}
...
export function BusinessQuoteEditor({ quote, locale, onApply, onClose, onSettings }: Props & { onSettings: () => void }) {
  ...
  return <EditorDialog title={text(locale, 'Votre entreprise sur ce devis', 'Business details for this Quote')} description={text(locale, 'Ces coordonnées et le statut TVA ne changent que ce brouillon. Les valeurs par défaut et les révisions publiées restent inchangées.', 'These details and VAT status change this Working Draft only. Defaults and Published Revisions stay unchanged.')} onClose={onClose}>
    <form className="flex max-h-[75vh] flex-col gap-4 overflow-y-auto" onSubmit={submit} noValidate>
      ...
      </FieldGroup><Actions locale={locale} onClose={onClose} />
    </form>
  </EditorDialog>;
}
...
export function SiteQuoteEditor({ quote, locale, onApply, onClose }: Props) {
  ...
  return <EditorDialog title={text(locale, 'Adresse du chantier', 'Site address')} description={text(locale, 'Le chantier peut se trouver à une autre adresse que le client.', 'The work site may differ from the Customer address.')} onClose={onClose}><form className="flex flex-col gap-4" onSubmit={e => { e.preventDefault(); onApply({ ...quote, siteAddress }); onClose(); }}><Button type="button" variant="outline" disabled={!quote.customerAddress.trim()} onClick={() => setSiteAddress(quote.customerAddress)}>{text(locale, 'Copier l’adresse du client', 'Copy Customer address')}</Button><FieldGroup><Field><FieldLabel htmlFor="site-address">{text(locale, 'Adresse du chantier', 'Site address')}</FieldLabel><Textarea id="site-address" value={siteAddress} onChange={e => setSiteAddress(e.target.value)} /></Field></FieldGroup><Actions locale={locale} onClose={onClose} /></form></EditorDialog>;
}
