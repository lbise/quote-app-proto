...
export function AssistantDisclosure({ locale, processing, onClose, onContinue }: { locale: 'fr' | 'en'; processing: QuoteAIDisclosure; onClose: () => void; onContinue?: () => void }) {
  ...
  return <Dialog open onOpenChange={open => { if (!open) onClose(); }}><DialogContent className="qp-modal"><DialogHeader>
    ...
    <DialogDescription>{disabled
      ...
      : t(`Easy Quote envoie votre message, la conversation utile et le contenu de travail du devis à ${processing.providerName} pour cette demande. Les champs dédiés aux noms, adresses, contacts, numéro TVA, référence, dates, adresse du chantier et conditions ne sont pas transmis. Les messages et descriptions peuvent toutefois contenir des données personnelles ou commerciales.`, `Easy Quote sends your message, the conversation needed to answer it, and the Quote work content to ${processing.providerName} for this request. Dedicated names, addresses, contacts, VAT identifier, reference, dates, work-site address, and terms are not sent. Messages and descriptions may still contain personal or commercial information.`)}</DialogDescription>
  </DialogHeader>
