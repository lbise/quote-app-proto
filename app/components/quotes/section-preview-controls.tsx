...
import { calculateQuote, type QuoteCalculation, type QuoteData } from '../../lib/quote';
import { MissingWarning, QuoteField } from './quote-validation';

type Locale = 'en' | 'fr';
const t = (locale: Locale, fr: string, en: string) => locale === 'fr' ? fr : en;

export function SectionHeading({ quote, calculation: previewCalculation, title, locale, readOnly, index, count, lineCount, onRename, onMove, onDuplicate, onRemove, onDelete, onAddLine, onInsert, changed }: {
  quote: QuoteData; calculation: QuoteCalculation; title: string; locale: Locale; readOnly: boolean; index: number; count: number; lineCount: number; changed: boolean;
  ...
  const calculation = renaming ? calculateQuote({ ...quote, sections: quote.sections.map((section, i) => i === index ? { ...section, title: draft } : section) }) : previewCalculation;
  const missing = calculation.missing.find(problem => problem.path === `sections[${index}].title`);
  ...
  return <div className="qp-section-title">
    {readOnly ? <h3>{title}</h3> : renaming ? <form className="qp-section-rename" onSubmit={rename}><QuoteField calculation={calculation} path={`sections[${index}].title`} id={`section-rename-${index}`} locale={locale}><label className="sr-only" htmlFor={`section-rename-${index}`}>{t(locale, `Nom de section ${title}`, `Section name ${title}`)}</label><Input id={`section-rename-${index}`} autoFocus value={draft} onChange={event => setDraft(event.target.value)} onKeyDown={event => { if (event.key === 'Escape') { event.preventDefault(); closeRename(); } }} /></QuoteField><Button size="sm" type="submit" disabled={!draft.trim()}>{t(locale, 'Enregistrer', 'Save')}</Button><Button type="button" size="sm" variant="ghost" onClick={closeRename}>{t(locale, 'Annuler', 'Cancel')}</Button></form> : <h3 aria-label={title}><button ref={headingButton} type="button" className="qp-section-rename-trigger qp-edit-target" aria-label={t(locale, `Renommer la section ${title}`, `Rename section ${title}`)} onClick={() => { setDraft(title); setRenaming(true); }}><span>{title}</span><EditAffordance /></button>{missing && <MissingWarning problem={missing} locale={locale} onClick={() => { setDraft(title); setRenaming(true); }} />}</h3>}
