  ...
  const chat = <section className="qp-conversation" aria-label={t('Conversation avec l’assistant', 'Conversation with assistant')}>
    ...
    <MessageScrollerProvider key={recordId} autoScroll defaultScrollPosition="last-anchor" scrollPreviousItemPeek={0}><MessageScroller className="qp-chat-scroller">
      <MessageScrollerViewport><MessageScrollerContent className="qp-chat-content">
        ...
        {toolDebug && <MessageScrollerItem><details className="qp-assistant-debug qp-assistant-tool-debug"><summary>{t(`${toolDebug.toolCalls.length} appel${toolDebug.toolCalls.length > 1 ? 's' : ''} d’outil réussi${toolDebug.toolCalls.length > 1 ? 's' : ''} (débogage)`, `${toolDebug.toolCalls.length} successful tool call${toolDebug.toolCalls.length > 1 ? 's' : ''} (debug)`)}</summary>{toolDebug.toolCalls.map((call, index) => <div key={`${call.name}-${index}`}><strong>{call.name}</strong><pre>{JSON.stringify(call, null, 2)}</pre></div>)}{toolDebug.requestId && <p>{t('Requête', 'Request')}: <code>{toolDebug.requestId}</code></p>}</details></MessageScrollerItem>}
