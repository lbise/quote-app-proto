...
export default function QuoteWorkspace({ initial, locale, onList, onLanguage }: { initial: QuoteRecord; locale: 'en' | 'fr'; onList: () => void; onLanguage: (locale: 'en' | 'fr') => void }) {
    ...
    <div ref={documentScroll} key={recordId} className="qp-document-scroll" role="region" tabIndex={0} aria-label={t('Contenu du devis, défilant', 'Scrollable Quote content')}>
      ...
      <article className="qp-paper" lang="fr">
        ...
        <div className="qp-totals" id="quote-totals">
          ...
          {quote.discountMode !== 'none' && quote.vatRegistered !== false && <div><span>Sous-total après remise HT</span><span>{formatMoney(sum.net)}</span></div>}
  ...
  return <div className="qp-app qp-variant-b" data-narrow-panel={narrowPanel} data-assistant-collapsed={assistantCollapsed} lang={locale}>
    ...
    {modal === 'publish' && <Dialog open onOpenChange={open => { if (!open) closeModal(); }}><DialogContent className="qp-modal" showCloseButton={false}><DialogHeader><DialogTitle>{t('Relire avant publication', 'Review before publication')}</DialogTitle><DialogDescription>{t('La publication fige le contenu. Elle n’envoie pas le devis.', 'Publication freezes the content. It does not send the Quote.')}</DialogDescription></DialogHeader>
      ...
      {blocked ? <Alert><TriangleAlert /><AlertTitle>{t('Publication indisponible', 'Publication unavailable')}</AlertTitle><AlertDescription>{t('Complétez les points signalés puis revenez à cette relecture.', 'Complete the flagged points, then return to this review.')}<ul>{save === 'error' && <li>{t('Les modifications ne sont pas enregistrées. Réessayez l’enregistrement avant de publier.', 'Changes are not saved. Retry saving before publishing.')}</li>}{save === 'saving' && <li>{t('Les modifications sont en cours d’enregistrement.', 'Changes are still being saved.')}</li>}{ai === 'processing' && <li>{t('Une modification de l’assistant est en cours. Attendez sa fin avant de publier.', 'An assistant change is in progress. Wait for it to finish before publishing.')}</li>}{[...calculation.errors, ...calculation.missing].map((problem, i) => <li key={i}>{problemLabel(problem, locale)}{<Button type="button" variant="link" size="sm" onClick={() => { closeModal(); setTimeout(() => revealField(problem.path), 0); }}>{t('Corriger', 'Fix')}</Button>}</li>)}</ul></AlertDescription></Alert> : <p>{t('Cette révision ne pourra être ni modifiée ni annulée. Une correction nécessitera une nouvelle révision.', 'This revision cannot be edited or undone. A correction requires a new revision.')}</p>}
