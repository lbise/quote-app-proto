...
describe.runIf(Boolean(process.env.TEST_DATABASE_URL))("authenticated Quote HTTP boundary", () => {
    ...
    try {
      ...
      const model = scriptedModel([
        fauxAssistantMessage([fauxToolCall("edit_quote_lines", {
          ...
          private_unexpected_field: "private-value-do-not-echo",
        })], { stopReason: "toolUse" }),
        fauxAssistantMessage([fauxToolCall("edit_quote_lines", { lines })], { stopReason: "toolUse" }),
  ...
  it("marks a delayed assistant response stale after an authenticated manual save", async () => {
    ...
    const delayed = scriptedModel([
      fauxAssistantMessage([fauxToolCall("edit_quote_details", { fields: { title: "Assistant title" }})], { stopReason: "toolUse" }),
  ...
  it("rejects all-work deletion as a whole-turn manual fallback", async () => {
    ...
    const model = scriptedModel([
      fauxAssistantMessage([
        fauxToolCall("edit_quote_details", { fields: { title: "Must not persist" }}),
  ...
  it("commits partial success below three failures and discards at the third failure", async () => {
    ...
    const partial = scriptedModel([
      fauxAssistantMessage([
        fauxToolCall("edit_quote_lines", { lines: [{ description: "Pose", mode: "fixed", quantity: "", unit: "", unitPrice: "", amount: "10.00" }]}),
    ...
    const exhausted = scriptedModel([
      fauxAssistantMessage([
        fauxToolCall("edit_quote_details", { fields: { title: "Encore" }}),
    ...
    expect(reloaded.draft.title).toBe("Titre sans source");
  });
});
