...
Each request creates a fresh agent and temporary Working Draft. A turn has at most twelve model responses, twenty-four tool-call attempts and three failed tool calls. The first and second failed calls are visible and do not discard successful staged calls; the third failed call stops and discards the whole turn. Tools execute sequentially and each call is atomic. The server validates and calculates the whole Quote, checks its version and commits one successful turn with one Undo target. Provider failure, invalid results, timeout or an exhausted budget discards staged changes. Request IDs prevent duplicate accepted changes, and manual saves invalidate stale assistant results.
