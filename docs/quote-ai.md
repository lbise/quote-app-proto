...
Do not write raw conversations, Quote descriptions, provider payloads or provider responses to application logs, traces or error-reporting breadcrumbs. When `QUOTE_AI_DEBUG=true`, a failed request exposes only a phase, stable reason code, tool name and request ID in the authenticated Quote UI; it does not expose prompts, arguments or provider responses. Enable this only while diagnosing a deployment and disable it afterward. The assistant cannot publish, send or accept a Quote. The Artisan reviews quantities, prices, technical content and applied changes before Publication.
