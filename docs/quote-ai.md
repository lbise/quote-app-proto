...
The model sends editable commercial values as typed tool arguments. New nonempty facts need grouped citations from the current Artisan message, retained Artisan messages or fields in the original Working Draft. Assistant replies cannot establish evidence. The application checks source identity and excerpt containment, field types, precision, bounds, pricing modes and whole-Quote validity. It does not prove a faithful rewrite or calculate or verify a model-derived quantity, formula or price adjustment. Evidence for a derived value records its source facts, not arithmetic proof. The application remains authoritative for line amounts, subtotals, Discount, VAT and totals. Unknown values stay empty rather than becoming zero. The assistant asks a focused question when the target or commercial fact is ambiguous.
