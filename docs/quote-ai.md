...
Do not write raw conversations, Quote descriptions, provider payloads or provider responses to application logs, traces or error-reporting breadcrumbs. The assistant cannot publish, send or accept a Quote. The Artisan reviews quantities, prices, technical content and applied changes before Publication.

The initial-capture tools are `read_work`, `set_customer_info`, `add_quote_line` and `supply_missing_line_fields`. They receive the authenticated Working Draft from trusted server context, not business or Quote identifiers chosen by the model. New lines get application-generated IDs. Follow-up may fill empty fields only on lines captured in this flow. General refinement, sections, discounts, duplication and title changes are not available in the current assistant flow.

Numeric mutations require evidence from the current Artisan message, retained Artisan messages or the same numeric field on an original Working Draft line. Assistant replies cannot establish evidence. Quantities and prices remain missing unless supplied. The application validates and calculates amounts. The assistant must ask focused clarification when a target or commercial fact is ambiguous.
