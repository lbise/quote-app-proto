...
Do not write raw conversations, Quote descriptions, provider payloads or provider responses to application logs, traces or error-reporting breadcrumbs. When `QUOTE_AI_DEBUG=true`, a failed request exposes a phase, stable reason code, tool name, the full failed tool call and request ID in the authenticated Quote UI. The tool call can contain Artisan-supplied or Customer data, so enable this only while diagnosing a deployment and disable it afterward; provider responses remain excluded. The assistant cannot publish, send or accept a Quote. The Artisan reviews quantities, prices, technical content and applied changes before Publication.
