...
```sh
npm run eval:db -- up
npm run eval:run -- --live --scenario joinery-full-reconstruction --provider-env-file .env --approve-provider-data-review --max-calls 9 --max-elapsed-ms 120000 --max-spend-usd 6 --database-url "$(bash scripts/eval-db.sh url)"
```
