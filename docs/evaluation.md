...
The library has two suites: six small fictional tool checks, one combined tool check and 27 scenario cases. The scenario suite prioritizes joinery/cladding and includes landscaping and civil works. Its commercial examples come from the reviewed adaptations in [`examples/first-quotes/`](examples/first-quotes/). Synthetic arithmetic and recovery cases are labelled separately. The joinery reconstruction accepts explicit French unit spellings such as `pièce` for `pce` and `m²` for `m2`; its revised scenario hash requires fresh live approval. A separate fictional scenario checks clarification before an ambiguous correction.
...
Live execution has dedicated bounded OpenRouter and direct Google boundaries. It is not a product benchmark and an offline pass is not evidence of model interpretation quality. Do not claim live evidence until a saved run has been inspected. Product fixes discovered by evaluation, final prompt/tool acceptance and product-owner sign-off remain unfinished parts of #29. #21 still owns production provider approval; #22 owns the external Artisan session. Evaluating multiple models does not approve any model for production. Production OpenRouter support is tracked in #38; session comparison is tracked separately in #36.
...
The home page shows saved Evaluation Sessions newest first, with model, execution status, saved automated outcomes and estimated usage cost. Expand **Scenario Runs and session details** to see individual results, or filter by scenario, automated outcome and live/offline mode. Older runs appear separately without invented session information; runs whose mode cannot be established say "Mode unavailable" rather than being counted as offline smoke. Open a run for execution issues and failed checks with expected/actual values. **Inspect step** opens the matching conversation step. Quote comparison, full diagnostics and scenario definitions remain in expandable sections; append-only human reviews remain below them. **Scenario library** contains versioned inputs and expectations, without a permanent sidebar on other pages. Database readiness and configured provider availability are shown without connection details or keys. Provider availability means the supported model and key are configured in the process environment, not that a paid request has succeeded. Choose **New evaluation** for a searchable scenario checklist beside model, reasoning and session-limit controls. Browse scenario inputs and expectations through **Preview**, then tick individual items or use **All scenarios** and **All tool tests** to add whole groups. Every checkbox remains editable afterward, so you can untick exceptions. Search and the Behavior filter narrow the list without clearing selections. **Select visible** adds displayed live-compatible cases. **All scenarios** and **All tool tests** add their respective suites, regardless of filters. **Clear** removes all selections. Behavior groups organize cases across trades and suites; they do not change scenario hashes, approval or automated outcomes. The header's light/dark toggle follows the system initially and saves an explicit choice in browser-local storage. It also works on the read-only review server. Each repetition is a separate planned Scenario Run; the default is one. Controlled-only cases cannot run live and are disabled in the launch checklist.

The dashboard has no login. In private-network mode it accepts only loopback or private-network peers and exact local interface Host headers; review submissions require the same origin. Restrict port 4320 to trusted devices with your firewall. Do not add a public proxy or port forward. The separate `eval:review` command still works for browsing and human reviews and has no execution routes. The evaluator started by `eval:start` also supports launch, progress and Stop, with the same Host and same-origin write checks as reviews. Artifacts remain in `.eval-artifacts/`, which is ignored by Git and excluded from production packaging. With the disposable database running, run `EVAL_DATABASE_URL="$(bash scripts/eval-db.sh url)" npx playwright test -c playwright-evaluator.config.ts` for browser coverage using a temporary evaluator server, never application credentials or provider calls.

## Authorize and monitor a browser evaluation

Configure `OPENROUTER_API_KEY` for OpenRouter and/or `QUOTE_AI_PROVIDER=google`, `QUOTE_AI_MODEL=gemini-3.5-flash-lite`, `GEMINI_API_KEY` for direct Google in the project's `.env` or the server process environment before running `npm run eval:start`. Startup reads only these provider settings and `QUOTE_AI_TIMEOUT_MS` from `.env` in the working directory. Exported variables take precedence, including explicitly empty values. It never loads database URLs, Node options or other settings from that file, and it does not change the process environment. A missing `.env` is allowed for browsing; an unreadable file stops startup with a safe error. The dashboard does not accept credentials or local paths from the browser. Missing credentials, unsupported configuration and expired pricing are explained before Start. `QUOTE_AI_TIMEOUT_MS` controls the product turn timeout separately from the session deadline.

Reasoning is a primary control. OpenRouter offers Off only when current model metadata establishes that `reasoning.effort=none` is supported across the routed endpoints. If Off is unavailable, choose an explicitly supported effort. Direct Gemini accepts minimal, low, medium and high; it cannot disable thinking. Advanced settings expose a model-validated combined output and thinking token limit. Set a provider-call limit per Scenario Run, a session-wide time limit in minutes, and a USD budget. The server also bounds total calls from the selected items and repetitions. The CLI retains its session-wide call limit. The browser and CLI share generation validation, price reservations and the execution supervisor.

Clicking **Start** authorizes the displayed selection, provider, exact model, settings and limits. There is no provider-data approval checkbox or confirmation modal. The saved plan records `browser-start`, the scenario hashes and launch time. This authorization does not change the library's input, expectation or provider-review flags, copy human approvals, or permit transmitting original source documents.

Start returns a session ID while the server continues execution. Its progress page polls saved state and shows the active scenario, completed-run count, reservations, results and sanitized errors. Navigating away or closing the tab does not stop execution. Reopen the session from history to see its progress or final state. A failed start remains in history. Duplicate submissions with the same request ID do not repeat paid work, and a competing launch is rejected rather than queued. The artifact-directory lock also applies to CLI processes.

**Stop** is safe to repeat. It attempts to abort ongoing provider work, keeps completed evidence and reservations, identifies interrupted work and skips the remainder. An interrupted Quote turn rolls back rather than partially committing. Cancellation does not guarantee the provider avoids charging. Stopping or restarting the server interrupts unfinished work; restarting never resumes provider calls automatically.
...
`--approve-provider-data-review` is CLI runtime authorization for the exact selected adapted scenario hashes, provider and model in this one session. It does not alter the library's `pending`, `blocked`, or reviewed flags. Scenario-input review, per-run human judgment and #21 production-provider approval are separate decisions. Browser Start is its own launch authorization and does not require this CLI flag or an equivalent checkbox. Existing CLI scripts remain compatible: live CLI execution still requires `--approve-provider-data-review`, explicit selection and all three limits. Direct Google requires explicit reasoning; OpenRouter defaults to Off only when the selected model supports it.

For direct Google the provider environment file needs `QUOTE_AI_PROVIDER=google`, `QUOTE_AI_MODEL=gemini-3.5-flash-lite`, and `GEMINI_API_KEY`. For OpenRouter it needs `OPENROUTER_API_KEY`, and the CLI takes `--provider openrouter --model author/model`. For the long reconstruction, consider `QUOTE_AI_TIMEOUT_MS=45000`; the application turn deadline otherwise defaults to 20 seconds and remains separate from the session deadline.
...
To evaluate one priced, compatible OpenRouter model, use the same limits with `--provider openrouter --model author/model --reasoning off` and a server-side `OPENROUTER_API_KEY`. Off is accepted only if live metadata proves it; otherwise choose a verified level. This is an example of the command shape, not approval to send real source documents or evidence of a paid run.

One durable session is shared by every selected scenario and repetition. Before provider work, the CLI saves its exact plan under `.eval-artifacts/sessions/`: scenario hashes, repetition IDs, model settings, authorization, limits and price snapshot. Mutable state records progress separately from immutable run artifacts. The CLI saves each completed run immediately, stops remaining work after cancellation or a limit, and exits nonzero for a failed run or stopped session. A failed start remains visible without an automated run. Reopening unfinished sessions marks them interrupted, never resumes them, and never restores an allowance. Only one session may own a given artifact directory at once, including competing CLI and browser launches. Review the saved artifacts with `npm run eval:review`; if that review server is already running, use the URL it printed rather than starting another server.

Direct Google remains limited to `gemini-3.5-flash-lite`. OpenRouter models come from the live platform catalog and endpoint metadata, not the bundled SDK catalog. A catalog listing is only a shortlist: before launch, the evaluator requires text input/output, custom tools, a common output-limit parameter across routed endpoints, supported reasoning, and complete conservative pricing. Some OpenRouter models will be unavailable with a reason; there is no model/provider fallback. The Google boundary pins a verified price snapshot that expires on **2026-09-29**. Renew the documented rates and expiry before then or live execution must fail closed.

The direct Google snapshot conservatively uses the highest published text rates: 540 nanodollars per input token and 4,500 per output token. These are Priority rates, although this boundary submits ordinary Standard requests. Before every provider call at the default output limit, the session irrevocably reserves the full 1,048,576 input tokens plus 4,096 output tokens: **$0.58466304**. A smaller configured output limit reduces only the per-call output reservation, not the input bound. It does not release a reservation, including when actual usage is smaller. The full reconstruction needs at least one response after its final tool batch. Eight successful tool calls therefore need a ninth provider call for the final response. Use a nine-call, `$6` cap for that run. Nine calls reserve at most **$5.26196736**. The full invocation is capped, not each scenario.

OpenRouter reservations use the smallest published context/output bounds and the highest charge among every published endpoint and pricing override, including cache, reasoning and per-request charges. The price snapshot, source, timestamp, endpoint assumptions and actual routed identity when exposed by the stream are kept with the session and run. A request uses `provider.require_parameters=true` and disables routing fallbacks, but the selected upstream provider is not guaranteed to be reported. Unknown routing or provider-internal behavior must not be described as verified. The snapshot is valid for 15 minutes and is checked before each call. Missing/unaccountable usage, incomplete streams or stale metadata stop further requests without returning the reservation. The estimate counts reasoning as a subset of output unless a separate reasoning price applies; it never adds reasoning tokens to output twice. Neither the cap nor estimates promise the platform's eventual invoice or cover other requests on the same account.

Both boundaries use no retries. `--reasoning minimal|low|medium|high` is mandatory for the supported Google model: its thinking cannot be turned off, so `off` is rejected rather than silently becoming minimal. `--max-output-tokens N` sets the advanced combined output and thinking limit (1–4096; default 4096). Both settings reach the real Agent and Google request. Production keeps its own provider approval and generation defaults. The recorded effective setting describes the validated request; provider-internal behavior beyond the submitted payload is unknown. Missing, partial, malformed, errored, or aborted usage stops the session and retains its reservation. Artifacts record estimated usage separately from reserved upper bounds. This is a bound under the recorded document rates, not a promise about a provider invoice or an account-wide spending guarantee.
...
Saved live calls show the SDK stop reason and provider finish reason when available. OpenRouter HTTP failures also retain the status and, when identifiable, a fixed error category or a known unsupported parameter name. Raw provider error messages and metadata are never saved: they may echo credentials or Quote content. A 400 without a narrower category means only that the request was rejected, not which setting was invalid. A call marked `complete` has usable usage accounting, not necessarily a successful Quote edit. Google `MAX_TOKENS` maps to SDK `length`; the application reports `assistant_output_limit_exceeded` and discards the whole turn without requesting another model response. Developer details also show these terminal codes. Older artifacts lack this metadata and display `not recorded`; their missing reasons cannot be reconstructed from the report.
...
| `contract-targeted-correction` | Correct one existing line while leaving another intact. |
| `contract-copy-unknown-quantity` | Copy known work but leave the new quantity missing while retaining the unit price. |
...
npm run eval:run -- --live --suite contract \
  ...
  --max-calls 24 --max-elapsed-ms 120000 --max-spend-usd 16 \
  --database-url "$(bash scripts/eval-db.sh url)"
```

The 24-call limit is shared across all seven checks, not granted to each one. At the recorded rates, 24 calls reserve at most USD 14.03191296, not an actual charge. Repetitions share that same invocation budget. No full scenario runs automatically after these checks.
...
Use `--suite contract --scenario contract-fixed-line` for one check. A scenario ID outside the selected suite is an error. `--suite scenario` previews the 27 cases; select explicit IDs for live execution because selections containing `controlled-only` fault-injection cases are rejected.
...
Both must pass for the automated run to pass. French wording, faithful interpretation and invented commitments still require human review. Six checks cover individual operations. The seventh combines them under a longer input and multiple batches. Clarification of an ambiguous target is a focused scenario (`ambiguous-panel-correction`), not a clean tool-call contract: the first turn asks a question without changing the Working Draft, and the second applies the specified correction. The automated check verifies a question mark and unchanged Quote; a human must still judge whether the question is useful. It does not force a particular batch partition or prove reliable interpretation of a 30-line Quote. Forced failures, transport errors, rollback and concurrency stay in controlled offline tests. Passing small live checks does not establish full-scenario quality.
...
2. Add a versioned `Scenario` in `eval/scenarios.ts`, or a fictional contract check in `eval/contract-scenarios.ts`. Add its browsing group in `eval/case-groups.ts`; groups are not part of a scenario's execution or authorization hash. Contract checks tag protocol assertions with `category: "contract"`; untagged assertions check commercial state. Record interface language, starting Working Draft, scripted Artisan messages, manual saves, clarification requirements and prohibited changes. Supply adapted units explicitly. Never let another model invent follow-up facts or consent.
