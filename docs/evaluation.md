...
Reasoning is a primary control. OpenRouter offers Off only when current model metadata establishes that `reasoning.effort=none` is supported across the routed endpoints. If Off is unavailable, choose an explicitly supported effort. Direct Gemini accepts minimal, low, medium and high; it cannot disable thinking. Advanced settings expose a model-validated combined output and thinking token limit. Set positive bounded session-wide call, elapsed-time and USD limits. The browser and CLI share generation validation, price reservations and the execution supervisor.
