...
Configure `QUOTE_AI_PROVIDER=google`, `QUOTE_AI_MODEL=gemini-3.5-flash-lite` and `GEMINI_API_KEY` in the project's `.env` or the server process environment before running `npm run eval:start`. Startup reads only those three settings and `QUOTE_AI_TIMEOUT_MS` from `.env` in the working directory. Exported variables take precedence, including explicitly empty values. It never loads database URLs, Node options or other settings from that file, and it does not change the process environment. A missing `.env` is allowed for browsing; an unreadable file stops startup with a safe error. The dashboard does not accept credentials or local paths from the browser. Missing credentials, unsupported configuration and expired pricing are explained before Start. `QUOTE_AI_TIMEOUT_MS` controls the product turn timeout separately from the session deadline.

Reasoning is a primary control with the validated choices minimal, low, medium and high. This model cannot disable thinking. Advanced settings expose the combined output and thinking token limit, 1–4096. Set positive bounded session-wide call, elapsed-time and USD limits. The browser and CLI share generation validation, price reservations and the execution supervisor.

Clicking **Start** authorizes the displayed selection, Google model, settings and limits. There is no provider-data approval checkbox or confirmation modal. The saved plan records `browser-start`, the scenario hashes and launch time. This authorization does not change the library's input, expectation or provider-review flags, copy human approvals, or permit transmitting original source documents.
