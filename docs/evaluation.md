...
Configure `QUOTE_AI_PROVIDER=google`, `QUOTE_AI_MODEL=gemini-3.5-flash-lite` and `GEMINI_API_KEY` in the server process environment before running `npm run eval:start`. The dashboard does not accept credentials or local paths from the browser and does not load `.env` automatically. Missing credentials, unsupported configuration and expired pricing are explained before Start. `QUOTE_AI_TIMEOUT_MS` controls the product turn timeout separately from the session deadline.
