...
- `app/lib/quote-assistant.server.ts` runs pi's bounded model/tool loop with work context and explicit Customer details supplied in the current Artisan message. `app/lib/quote-tools.server.ts` stages flat initial capture, Quote-local Customer details, and missing-field clarification; it never exposes reusable Customer records. The server commits a successful turn as one undoable action. The assistant has no Publication authority. See [hosted AI configuration](quote-ai.md).
