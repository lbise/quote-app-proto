...
const expectedFocusedQuotes: Record<string, QuoteData> = {
  "ambiguous-panel-correction": complete("TEST-TARGET-001", { vatRegistered: false, vatId: "", lines: [line("east", "", "Panel installation, east wing", "quantity", "2", "m2", "10.00", ""), line("west", "", "Panel installation, west wing", "quantity", "4", "m2", "10.00", "")] }),
...
const scenarioLibrary: Scenario[] = [
  ...
  synthetic("ambiguous-panel-correction", "Clarify an ambiguous target before changing a price", "joinery", "en", complete("TEST-TARGET-001", { vatRegistered: false, vatId: "", lines: [line("east", "", "Panel installation, east wing", "quantity", "2", "m2", "10.00", ""), line("west", "", "Panel installation, west wing", "quantity", "3", "m2", "10.00", "")] }), [
    artisan("Change the panel area to 4 m2.", [unchanged("quote"), unchanged("quote.lines[0]"), equalsAssertion("ambiguous request changes nothing", "outcome", "unchanged"), contains("asks which area", "message", "?"), equalsAssertion("no rejected calls", "failedCalls", 0)]),
    artisan("I mean the west wing panels. Set those to 4 m2; leave the east wing alone.", [equalsAssertion("west corrected", "quote.lines[1].quantity", "4"), unchanged("quote.lines[0]"), equalsAssertion("total after correction", "calculation.total", 6_000), equalsAssertion("committed", "outcome", "committed"), equalsAssertion("no rejected calls", "failedCalls", 0)]),
  ], ["quote.lines[0]"], ["Check that the question identifies the competing targets and that no price or unrelated work was invented."]),
