...
function history(input: { executionEnabled?: boolean; sessions: EvaluationSessionRecord[]; runs: EvaluationRun[]; reviewsByRun?: Record<string, HumanReview[]>; scenarioId?: string; outcome?: string; mode?: string; dashboardStatus?: DashboardStatus }): string {
  ...
  const sessionCards = sessions.map(session => {
    ...
    return `<details class="session"${scenario || outcome ? " open" : ""}><summary><strong>${h(session.plan.id)}</strong> · ${h(session.plan.createdAt)} · ${h(session.plan.mode === "live" ? "Live model" : "Offline smoke")} · Execution: ${h(session.state.status)}</summary>
      ${input.executionEnabled ? `<p><a href="/?session=${encodeURIComponent(session.plan.id)}">Open progress and results</a> · <a href="/?reuse=${encodeURIComponent(session.plan.id)}">Reuse selection and settings</a></p>` : ""}
...
export function renderReport(input: ExecutionView & { scenarios: Scenario[]; runs: EvaluationRun[]; sessions?: EvaluationSessionRecord[]; dashboardStatus?: DashboardStatus; outcome?: string; mode?: string; scenarioId?: string; runId?: string; historyView?: boolean; reviewsByRun?: Record<string, HumanReview[]>; reviews: HumanReview[] }): string {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Quote evaluation review</title><link rel="stylesheet" href="/report.css">${input.executionEnabled ? '<script src="/execution.js" defer></script>' : ""}</head><body>
    <header class="masthead"><a href="/">Easy Quote / evaluation</a><p>Private local report. Retained commercial prices and technical details. ${input.executionEnabled ? 'Only an explicit Start authorizes provider calls.</p><p><a href="/?launch=1">New evaluation</a>' : "No provider calls from this browser."}</p></header>
    ...
    <main>${input.executionEnabled && input.launchView ? launchForm(input) : input.executionEnabled && input.sessionId ? (sessions.find(item => item.plan.id === input.sessionId) ? sessionProgress(sessions.find(item => item.plan.id === input.sessionId)!, input.runs) : '<h1>Session not found</h1><p>Return to history to choose a saved session.</p>') : input.historyView ? `${!scenario ? "<p>No scenarios available</p>" : ""}${history({ executionEnabled: input.executionEnabled, sessions, runs: input.runs, scenarioId: input.scenarioId, outcome: input.outcome, mode: input.mode, dashboardStatus: input.dashboardStatus, reviewsByRun: input.reviewsByRun })}` : scenario ? `<header><p>${h(scenario.provenance.kind)} · ${h(scenario.profession)} · v${scenario.version} · ${h(scenario.locale)}${scenario.execution === "controlled-only" ? " · Controlled fault injection only" : ""}</p><h1>${h(scenario.title)}</h1>${scenarioSuite(scenario) === "contract" ? "<p class=notice>Fictional contract check for edit behavior; it is not a commercial or Artisan-work example.</p>" : ""}<p>${run?.live ? "Library defaults. " : ""}Provider use: ${h(scenario.review.provider)} · Inputs: ${h(scenario.review.inputs)} · Expectations: ${h(scenario.review.expectations)}</p><p>${h(scenario.review.note)}</p>${input.executionEnabled && !run && scenario.execution !== "controlled-only" ? `<p><a href="/?launch=1&amp;scenario=${encodeURIComponent(scenario.id)}">Run this scenario</a></p>` : ""}</header>
